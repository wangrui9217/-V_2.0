  ...
  <div>
    <div class="g-listpage__style">
      <el-tabs v-model="activeName">
        <el-tab-pane label="待确认放款申请" name="first" >
          <div>
            ...
            <el-table
              ...
              <el-table-column
                prop="loanTime"
                label="放款申请时间"
                :formatter="(row) => $formatDate(row.loanApplyTime, 'yyyy-MM-dd')"
              >
              </el-table-column>
              <el-table-column label="操作" width="80">
                <template slot-scope="scope">
                  <el-button type="text" @click="loanApply(scope.row)">处理</el-button>
                </template>
              </el-table-column>
            </el-table>
            <el-pagination
        ...
        <el-tab-pane label="已确认放款申请" name="second" >
          <div class="search-group">
            <div></div>
            <div class="right">
          ...
          <el-table
            ...
            <el-table-column label="操作" width="80">
              <template slot-scope="scope">
                <el-button type="text" @click="loanApply(scope.row, 'hide')">查看</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      loanApply (row, flag) {
        const temp = {orderId: row.businessKey, definitionKey: row.definitionKey, node: row.name, processInstanceId: row.processInstanceId}
        const type = {type: 'check'}
        const query = flag ? {...temp, ...type} : temp
        this.$router.push({name: 'loanDetail', query: query})
      },
      // 搜索分页
      ...
      init () {
        ...
        lendController.todoList(params).then(res => {
          if (res.status === 200) {
      ...
      init2 () {
        ...
        lendController.alreadyCheckList(params).then(res => {
          if (res.status === 200) {
