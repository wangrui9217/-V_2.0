<template>
  <el-form :model="ruleForm2" status-icon ref="ruleForm2" label-width="140px" class="demo-ruleForm">
    <el-form-item label="参数key" prop="code"  :rules='$rules.requiredInputValue'>
      <el-input  v-model="ruleForm2.code" :disabled="isEdit"></el-input>
    </el-form-item>
    <el-form-item label="参数名称" prop="value"  :rules='$rules.requiredInputValue'>
      <el-input  v-model="ruleForm2.value" ></el-input>
    </el-form-item>
    <el-form-item label="参数值" prop="name"  :rules='$rules.requiredInputValue'>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        addCar () {
          this.dataFormat()
          dictionaryController.saveDictionary(this.ruleForm2).then(res => {
            if (res.status === 200) {
