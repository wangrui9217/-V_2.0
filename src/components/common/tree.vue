<template>
  <div class="tree-container">
    <ul id = "menu" v-show="menuVisible" @click="hideMenu">
      ...
      <li @click="menuRemove" :class="{'not-allowed': chosedNodeId === 0}">删除该类型</li>
      <li @click="menuChange"  :class="{'not-allowed': chosedNodeId === 0}">修改名称</li>
    </ul>
    <div class="box">
      <div class="inner">
        <el-input
        ...
        <el-tree
          ...
          :default-expanded-keys="defaultExpandedKeyList"
          ref="tree"
...
export default {
  ...
  methods: {
    ...
    getTreeData() {
      ...
      return new Promise((resolve, reject) => {
        api.getSelectTypeList(params).then(res => {
          ...
          this.data = temp2;
          if (this.type == 2) {
            this.$emit("refreshCategory", this.data);
          }
    ...
    nodeExpand(val) {
      this.defaultExpandedKeyList.push(val.categoryId);
    },
    nodeCollapse(val) {
      var index = this.defaultExpandedKeyList.indexOf(val.categoryId);
      this.defaultExpandedKeyList.splice(index, 1);
    },
    ...
    rihgtClick(event, object, value, element) {
      if (this.type == 3) {
        ...
        menu.style.top =
          event.clientY -
          this.doomObj.y +
          document.getElementsByClassName("admin_mian_scrollbar")[0].children[0]
            .scrollTop +
          "px";
    ...
    renderContent(h, { node, data, store }) {
      return (
        <span class="custom-tree-node">
          <i class="fa fa-align-justify my-icon" />
  ...
  mounted() {
    this.getTreeData().then(() => {
      this.chosedNodeId =
        this.$route.params.selected && this.$route.params.selected !== "null"
          ? this.$route.params.selected
          : 0;
      // console.log(this.chosedNodeId)
      // this.chosedNodeId = 0
      this.$refs.tree.setCurrentKey(this.chosedNodeId);
    });
    this.$nextTick(() => {
